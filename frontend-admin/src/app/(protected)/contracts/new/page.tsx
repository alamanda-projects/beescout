...
const schema = z.object({
  ...
  metadata: z.object({
    ...
    // bukan di sla. Format ISO (YYYY-MM-DD dari <input type="date">).
    ...
    // (read-path lenient), enforcement berlapis di FE zod + write-path backend.
    sla: z.object({
...
const AVAIL_UNIT_LABEL: Record<string, string> = { h: 'jam', d: 'hari' }
const FREQ_UNIT_LABEL: Record<string, string> = { m: 'menit', h: 'jam', d: 'hari' }
...
function ContractReview({ data, retentionValue, retentionUnit }: {
  ...
  const consumers = data.metadata?.consumer ?? []
  ...
  return (
    <div className="space-y-5">
      ...
      <ReviewSection title="SLA">
        {(() => {
          const s = data.metadata?.sla as any
          const avStart = s?.availability_start
          const avEnd = s?.availability_end
          const avUnit = s?.availability_unit
          const freq = s?.frequency
          const freqUnit = s?.frequency_unit
          return (
            <>
              <ReviewRow label="Ketersediaan" value={avStart != null && avEnd != null ? `${avStart}–${avEnd} ${AVAIL_UNIT_LABEL[avUnit] ?? avUnit}` : ''} />
              <ReviewRow label="Frekuensi Update" value={freq != null ? `${freq} ${FREQ_UNIT_LABEL[freqUnit] ?? freqUnit}` : ''} />
              <ReviewRow label="Retensi Data" value={retentionValue ? `${retentionValue} ${retentionUnit}` : ''} />
              <ReviewRow label="Jadwal Cron" value={s?.frequency_cron} />
            </>
          )
        })()}
      </ReviewSection>
      ...
      <ReviewSection title={`Konsumen (${consumers.filter(c => c.name?.trim()).length})`}>
        {consumers.filter(c => c.name?.trim()).length === 0 ? (
          <p className="text-sm text-muted-foreground italic">Belum ada konsumen terdokumentasi.</p>
        ) : consumers.filter(c => c.name?.trim()).map((c, i) => (
          <div key={i} className="text-sm">
            <span className="font-medium">{c.name}</span>
            {c.use_case && <span className="text-muted-foreground"> · {c.use_case}</span>}
          </div>
        ))}
      </ReviewSection>
...
export default function NewContractPage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      metadata: {
        ...
        sla: { availability_start: undefined as any, availability_end: undefined as any, availability_unit: 'h' as 'h' | 'd', frequency: undefined as any, frequency_unit: 'h' as 'm' | 'h' | 'd', frequency_cron: '', retention: '' },
        stakeholders: [],
        consumer: [],
        quality: [],
      },
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      // Build SLA — semua spec fields dikirim as-is; retention via local state.
      const sla = data.metadata.sla ?? {}
      const slaPayload = {
        availability_start: (sla as any).availability_start,
        availability_end: (sla as any).availability_end,
        availability_unit: (sla as any).availability_unit,
        frequency: (sla as any).frequency,
        frequency_unit: (sla as any).frequency_unit,
        ...((sla as any).frequency_cron ? { frequency_cron: (sla as any).frequency_cron } : {}),
