...
export function Header() {
  ...
  const { data: user } = useQuery({ queryKey: ['me'], queryFn: getMe, staleTime: 5 * 60 * 1000 })
  ...
  const initials = user?.client_id?.charAt(0)?.toUpperCase() ?? 'A'
  const roleLabel = user?.group_access ? ROLE_LABELS[user.group_access] : ''

  return (
    <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0">
      <div className="flex items-center gap-2">
        <Badge variant="warning" className="text-xs">Admin Portal</Badge>
      </div>
