...
- ✅ **Slice 4 — `model.description`** (PR #128): `requiredString` di 4 form (label `*` + error inline) + write-time 422 (extend loop model slice 3). **Dengan backfill migration** `migrate_backfill_model_description.py` (+ Makefile target) — isi description kosong dengan penanda `(Otomatis dari nama: …) — mohon dilengkapi`, idempoten, dry-run default; jalankan di prod supaya kontrak legacy tidak ke-block saat edit. Test migration + write-check.
